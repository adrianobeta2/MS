...
def carregar_imagem(caminho, coordenadas):
    ...
    roi = img[y:y+h, x:x+w]
    roi = cv2.resize(roi, (64,64))   # 🔹 agora 64x64
    roi = roi.astype("float32") / 255.0
    return roi.reshape(64,64,1)      # 🔹 ajusta formato para CNN
...
def treinar(camera, programa):
    ...
    # Listar imagens OK e NOK (sem mudar seu estilo)
    template_images_ok = [
    ...
    for i in range(1, n_rois+1):
        ...
        if section_name in config:
            ...
            model = Sequential([
                ...
                Dropout(0.5),   # 🔹 ajuda a reduzir overfitting
            ...
            model.fit(datagen.flow(x, y, batch_size=2), epochs=30)
