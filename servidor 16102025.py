...
def carregar_imagem(imagem, coordenadas):
    ...
    return roi.reshape(1,28,28,1)   # 🔹 ajusta o shape para o modelo
...
def predizer_frame(camera, programa,roi, gray,coordenadas):
    ...
    if img is None or img.size == 0:
     raise ValueError(f"Erro: ROI {coordenadas} não retornou imagem válida.")
    
    print("Tipo de retorno carregar_imagem:", type(img))
    if img is not None:
     print("Shape inicial da imagem:", img.shape)
    ...
    def generate_frames_wecam():
        while True:
            if cam_tipo == "webcam":
                ...
                frame = monitorar_movimentos_rasp.monitor_movimento(frame)
            else:
