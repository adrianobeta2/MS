...
import treinar_modelo_new
import monitorar_movimentos
import facial
import logging
import subprocess
...
def get_serial():
    try:
        ...
        # Primeiro tenta o caminho do Jetson Orin / Jetson Nano
        ...
        # Se não achar (por exemplo, se for Raspberry Pi), tenta o /proc/cpuinfo
...
cam_2 = config.get("CAMERAS", "cam_2") #comando para listar as cameras disponiveis:  ls -l /dev/v4l/by-id/
cam_3 = config.get("CAMERAS", "cam_3") #comando para listar as cameras disponiveis:  ls -l /dev/v4l/by-id/
...
CAMERA_SERIALS = [serial_number_1, serial_number_2,serial_number_3]
cameras_web = {}
CAMERA_IDS_WEB = [cam_1, cam_2,cam_3]  # cam_1 e cam_2 são índices (ex: 0, 1)
...
def get_latest_webcam_frame_url(url):
...
def desenhar_roi_old(imagem, x, y, w, h, status, posicao, forma="retangulo"):
    """
...
# Função para desenhar a ROI (retângulo ou círculo) e escrever a label
def desenhar_roi(imagem, x, y, w, h, status, posicao, forma="retangulo", ativar_label = True, label="ROI"):
    """
    Desenha uma ROI na imagem e escreve a label ao lado dela.
    
    :param imagem: Imagem onde a ROI será desenhada.
    :param x, y: Coordenadas do canto superior esquerdo (retângulo) ou centro (círculo).
    :param w, h: Largura e altura da ROI (para retângulo) ou raio (para círculo).
    :param status: Status da ROI (True ou False).
    :param posicao: Define cor especial (exemplo: azul).
    :param forma: Forma da ROI ("retangulo" ou "circulo").
    :param label: Texto da label a ser desenhada.
    """

    if posicao:
        cor_roi = (255, 0, 0)  # Azul
    else:
        cor_roi = (0, 255, 0) if status else (0, 0, 255)  # Verde ou vermelho

    # Desenhar a ROI
    if forma == "retangulo":
        cv2.rectangle(imagem, (x, y), (x + w, y + h), cor_roi, 1)
        # posição do texto: à direita do retângulo
        text_x = x + w + 5
        text_y = y + int(h / 2)
    elif forma == "circulo":
        centro = (x, y)
        raio = w
        cv2.circle(imagem, centro, raio, cor_roi, 1)
        # posição do texto: à direita do círculo
        text_x = x + raio + 5
        text_y = y
    else:
        raise ValueError("Forma não suportada. Use 'retangulo' ou 'circulo'.")
...
LOG_PATH = os.path.expanduser("~/restart_jetson.log")
...
def reboot():
   #token = request.headers.get("Authorization")
    ...
    return jsonify({
        "status": "ok",
        "message": "Reboot agendado para 5 segundos"
    }), 200
...
def carregar_imagem(imagem, coordenadas):
    ...
    roi = cv2.resize(roi, (28,28)).astype("float32") / 255.0
    return roi.reshape(1,28,28,1)

def carregar_modelo(camera, programa, roi):
    chave = f"{camera}_{programa}_{roi}"
    if chave not in MODELOS:
        caminho = f"modelo_cam{camera}_prog{programa}_ROI{roi}.keras"
        MODELOS[chave] = load_model(caminho)
        print(f"✅ Modelo {chave} carregado na GPU.")
    return MODELOS[chave]

def predizer_frame(camera, programa, roi, gray, coordenadas):
    model = carregar_modelo(camera, programa, roi)
    img = carregar_imagem(gray, coordenadas)
    pred = model.predict(img, verbose=0)
    return bool(pred[0][0] >= 0.5)

def predizer_frame_new(camera, programa, roi, frame, coordenadas, threshold=0.5):
    """
    Predição OK / NOK usando TensorFlow + Transfer Learning

    :param camera: id da câmera
    :param programa: id do programa
    :param roi: índice da ROI
    :param frame: frame OpenCV (BGR)
    :param coordenadas: (x, y, w, h)
    :param threshold: limiar de decisão
    ...
    # --- Predição ---
    prob_ok = float(model.predict(roi_img, verbose=0)[0][0])
...
def treinar():
    try:
       ...
       #section = data.get('section') # obtem a seção (ROI) do JSON, se fornecida.
        ...
        #model_sumary = treinar_modelo_new.treinar(
...
url_cam3 = f"http://127.0.0.1:6001/capture/{serial_number_2}"
...
url_web_cam3 = f"http://127.0.0.1:6001/capture_webcam/{cam3_encoded}"
...
def processResult():
    try:
        ...
        if(camera == '1'):
            ...
            if(cam_tipo == "basler"):
              return jsonify({"error": f"Erro. Pendente parametro camera basler: exemplo: camera:1 e camera:2"})
              # serial_camera = serial_number_2
            else:
              url_cam = url_web_cam3
        else:
        ...
        modelo_yol = YOLO(f"runs/detect/treinos/cam{camera}_prog{programa}/weights/best.pt")
        #modelo_yol = YOLO(config['Modelos']['cabo_flex'])
        ...
        for i, (x, y, w, h,textura_tolerancia,pixel_tolerancia, threshold_cor,ponto_tolerancia) in enumerate(rois, start=1):
            ...
            if rede_neural == True:
                ...
                #status_rna = predizer_frame(camera,programa,roi_idex,gray,coordenadas)
            ...
            if pixel:
                ...
                status_ok, media_roi = AnalisePixel.new_pixel_analise_OK(
                    roi, camera, programa, gray, mask, coordenadas,
                    pixel_tolerancia, contrast, brightness, gamma, exposure
                )

                if status_ok:
                    status_roi = True   # APROVADO

                else:
                    status_nok, media_roi = AnalisePixel.new_pixel_analise_NOK(
                        roi, camera, programa, gray, mask, coordenadas,
                        pixel_tolerancia, contrast, brightness, gamma, exposure
                    )

                    if status_nok:
                        status_roi = False  # REPROVADO
                    else:
                        status_roi = False  # INDEFINIDO = REPROVADO
            else:
                status_roi = True
            ...
            desenhar_roi(image, x, y, w, h, status, False, forma, True, f'ROI{i}')   

            status_list.append(status)  # Armazena o status para verificar depois
            
            if posicao:
                x_t, y_t = roi_template
                desenhar_roi(image, x_t, y_t, width_ref, height_ref, status, posicao, "retangulo",True, f'Localizador')
            rois_data.append({
                "roi_index": i,
    ...
    try:
        ...
        if (camera =='1'):
             ...
             if(cam_tipo == "basler"):
                 ...
                 response = requests.get(url_web_cam3)     
        else:
...
def get_cameras_config():
    ...
    return jsonify({
        ...
        "cam_3": config['CAMERAS'].get('cam_3'),
        ...
        "cam_basler_3": config['CAMERAS'].get('serial_number_basler_3')
    })
...
total_clusters_carrinho = 0
tempo_berco_1_ocupado = 0
tempo_berco_2_ocupado = 0  
frame_anotado_pessoas = None
frame_anotado_carrinho = None
frame_anotado_bercos = None
frame_anotado_facial = None 
tempo_sem_objeto = 0
total_operadores = 0
cluster_no_berco =False
contador_total_clusters_berco = 0
tempo_cluster_no_berco = 0
processed_frames = {}
processed_frames_pessoas = {}
processed_frames_carrinho = {}
processing_threads = {}
def yolo_processing_thread(cam_key):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key}")
    global cluster_no_berco,tempo_sem_objeto,contador_total_clusters_berco,tempo_cluster_no_berco,tempo_berco_1_ocupado,tempo_berco_2_ocupado,total_clusters_carrinho, frame_anotado_bercos
    while True:
        try:
            ...
            # Aplica o processamento YOLO (sua função)
            frame_anotado = frame_anotado_bercos#, tempo_sem_objeto, cluster_no_berco, contador_total_clusters_berco, tempo_cluster_no_berco, tempo_berco_1_ocupado, tempo_berco_2_ocupado = monitorar_movimentos.monitor_movimento(frame,camera,programa)

            # Armazena o resultado anotado
            processed_frames[cam_key] = frame_anotado

        except Exception as e:
            print(f"[YOLO] Erro processando {cam_key}: {e}")
            time.sleep(0.5)

def yolo_processing_thread_operadores(cam_key):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key}")
    global total_operadores, frame_anotado_pessoas
    while True:
        try:
            
            time.sleep(0.01)
            # Aplica o processamento YOLO (sua função)
            frame_anotado = frame_anotado_pessoas#, total_operadores = monitorar_movimentos.monitor_movimento_pessoas(frame,camera,programa)
            
            # Armazena o resultado anotado
            processed_frames_pessoas[cam_key] = frame_anotado

        except Exception as e:
            print(f"[YOLO] Erro processando {cam_key}: {e}")
            time.sleep(0.5)  

def yolo_processing_thread_carrinho(cam_key):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key}")
    global frame_anotado_carrinho
    while True:
        try:
            
            time.sleep(0.01)
            # Aplica o processamento YOLO (sua função)
            frame_anotado = frame_anotado_carrinho#, total_operadores = monitorar_movimentos.monitor_movimento_pessoas(frame,camera,programa)
            
            # Armazena o resultado anotado
            processed_frames_carrinho[cam_key] = frame_anotado
...
import time as t
import datetime
global turno_agora, turno_anterior, turno_atual
turno_atual = "iniciando"
turno_agora = None
turno_anterior = None
def obter_turno():
    ...
    while True: 
        try:   
            ...
            # Definição dos horários de cada turno
            turno1_inicio = datetime.time(7, 0)
            turno1_fim    = datetime.time(16, 50)

            turno2_inicio = datetime.time(16, 50)
            turno2_fim    = datetime.time(22, 0)
            turno3_inicio = datetime.time(22, 0)
            turno3_fim    = datetime.time(7, 0)  
            ...
            # Turno 2
            if turno2_inicio <= agora < turno2_fim:
                turno_atual = "2º Turno"
            

            # Turno 3 (começa 22:00 e vai até 07:00 do próximo dia)
            if agora >= turno3_inicio or agora < turno3_fim:
                turno_atual = "3º Turno"
            time.sleep(1)  # Verifica a cada 1 segundos
        except Exception as e:
            print(f"[ERRO] Erro ao obter turno: {e}")

# Inicia a thread de monitoramento de eventos
def monitoramento_turno():
    monitoring_thread = threading.Thread(target=obter_turno, daemon=True)
...
# Controle de cooldown (em segundos)
COOLDOWN_ALERTA = 120  # 2 minutos
...
ultimo_alerta = {
    ...
    "sem_produto": 0,
    "operadores": 0,
    "ops8_aguardando_producao": 0,
    "novo_turno": 0
}

def pode_disparar(chave):
    """Verifica se já passou o tempo mínimo para novo alerta"""
    agora = time.time()
    if agora - ultimo_alerta[chave] >= COOLDOWN_ALERTA:
        ultimo_alerta[chave] = agora
        return True
    return False

import subprocess
def tocar_beep(audio_file="beep_trim.wav"):
    def play():
        subprocess.Popen(
            ["aplay", audio_file],
...
def monitorar_eventos():
    global cluster_no_berco, turno_anterior,tempo_sem_objeto, total_clusters_carrinho, total_operadores, turno_atual, tempo_cluster_no_berco, tempo_berco_1_ocupado, tempo_berco_2_ocupado, contador_total_clusters_berco, matricula_atual
    ...
    while True:
        ...
        try:
            ...
            #turno_agora = obter_turno()

            if (turno_atual != turno_anterior):
                turno_anterior = turno_atual
                if pode_disparar("novo_turno"):
                    print(f"[ALERTA] Novo turno iniciado: {turno_atual}")
                    match turno_atual:
                        case "1º Turno":
                            tocar_beep("1_turno.wav")
                        case "2º Turno":
                            tocar_beep("2_turno.wav")
                        case "3º Turno":
                            tocar_beep("3_turno.wav")
                    #tocar_beep("novo_turno.wav")

            # --- CLUSTER NO CARRINHO ----
            if (cluster_no_berco and total_clusters_carrinho > 3 and total_operadores > 1 and tempo_cluster_no_berco > 25) and (tempo_berco_1_ocupado == 0 or tempo_berco_2_ocupado == 0):
             #if (cluster_no_berco and total_clusters_carrinho > 3  and tempo_cluster_no_berco > 25) and (tempo_berco_1_ocupado == 0 or tempo_berco_2_ocupado == 0):
                if pode_disparar("cluster_carrinho"):
                    print("[ALERTA] Clusters aguardando no carrinho!")
                    tocar_beep("carrinho.wav")
                    
            # --- CLUSTER NO BERÇO ---
            #if (cluster_no_berco and tempo_cluster_no_berco > 25 and total_operadores > 1): #10 segundos
            if (cluster_no_berco and tempo_cluster_no_berco > 15 ): #10 segundos
                if pode_disparar("cluster_berco"):
                    print("[ALERTA] Há produtos aguardando serem removidos do berço!") # apos 15 segundos alerta produto no berço
                    tocar_beep("produto_berco.wav")

            # --- REGISTRO DE DESEMPENHO NO DB---
                    registrar_desempenho_thread(matricula_atual, turno_atual, tempo_cluster_no_berco, "1", "1")

            # --- SEM PRODUTO NO BERÇO ---
            if tempo_sem_objeto > 200 and not cluster_no_berco and total_operadores == 0:
                if pode_disparar("sem_produto"):
                    print("[ALERTA] OP S8 aguardando produção!") # linha de produção parada
                    tocar_beep("ops8_aguardando_producao.wav")

            if tempo_berco_1_ocupado > 120: # 120 segundos - checha se o berço 1 está travado
                if pode_disparar("berco_1_ocupado"):
                    print("[ALERTA] Berço 1 ocupado por muito tempo!")
                    tocar_beep("berco1.wav")
            if tempo_berco_2_ocupado > 120: # 120 segundos - checha se o berço 2 está travado
                if pode_disparar("berco_2_ocupado"):
                    print("[ALERTA] Berço 2 ocupado por muito tempo!")
                    tocar_beep("berco2.wav")
...
# Inicia a thread de monitoramento de eventos
def inicializa_monitoramento():
    monitoring_thread = threading.Thread(target=monitorar_eventos, daemon=True)
    monitoring_thread.start()

######################################### cam 1 - monitoramento do berço #############################################################

def yolo_processing_thread_cam1(cam_key, camera, programa):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key}")
    global cluster_no_berco,tempo_sem_objeto, contador_total_clusters_berco,tempo_cluster_no_berco, tempo_berco_1_ocupado, tempo_berco_2_ocupado, frame_anotado_bercos
    ...
    while True:
        try:
            ...
            frame_anotado_bercos, tempo_sem_objeto, cluster_no_berco, contador_total_clusters_berco, tempo_cluster_no_berco, tempo_berco_1_ocupado, tempo_berco_2_ocupado = monitorar_movimentos.monitor_movimento(frame,camera,programa)
            
            # Armazena o resultado anotado
            processed_frames[cam_key] = frame_anotado_bercos
            #annotated_resized = cv2.resize(frame_anotado, (new_w, new_h))
             # Exibe
...
def inicializa_yolo_cam_1(cam_key,camera,programa):
     t = threading.Thread(target=yolo_processing_thread_cam1, args=(cam_key, camera, programa), daemon=True)
     t.start()

##### cam 2 - monitoramento do carrinho buffer ######################################################################################
def yolo_processing_thread_cam2(cam_key_cam2, camera_cam2, programa_cam2):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key_cam2}")
    global total_clusters_carrinho, frame_anotado_carrinho,processed_frames_carrinho
    while True:
        try:
            frame_cam2 = get_latest_webcam_frame(cam_key_cam2)
            if frame_cam2 is not None:
                    frame_cam2 = cv2.resize(
                        frame_cam2,
                        (TARGET_WIDTH, TARGET_HEIGHT),
                        interpolation=cv2.INTER_AREA
                    )
            if frame_cam2   is None:
                time.sleep(0.01)
                continue

            # Aplica o processamento YOLO (sua função)
            frame_anotado_carrinho, total_clusters_carrinho = monitorar_movimentos.monitor_movimento_carrinho(frame_cam2,camera_cam2,programa_cam2)

            # Armazena o resultado anotado
            processed_frames_carrinho[cam_key_cam2] = frame_anotado_carrinho

        except Exception as e:
            print(f"[YOLO] Erro processando {cam_key_cam2}: {e}")
            time.sleep(0.5)



def inicializa_yolo_cam_2(cam_key_cam2,camera_cam2,programa_cam2):
     t = threading.Thread(target=yolo_processing_thread_cam2, args=(cam_key_cam2, camera_cam2, programa_cam2), daemon=True)
     t.start()


##### cam 3 - monitoramento dos operadores ######################################################################################
def yolo_processing_thread_cam3(cam_key, camera, programa):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key}")
    global total_operadores, frame_anotado_pessoas
    while True:
        try:
            ...
            # Aplica o processamento YOLO (sua função)
            frame_anotado_pessoas, total_operadores = monitorar_movimentos.monitor_movimento_pessoas(frame,camera,programa)

            # Armazena o resultado anotado
            processed_frames[cam_key] = frame_anotado_pessoas
...
def inicializa_yolo_cam_3(cam_key,camera,programa):
     t = threading.Thread(target=yolo_processing_thread_cam3, args=(cam_key, camera, programa), daemon=True)
     t.start()


@app.route('/video_feed_webcam_yolo/<path:serial_number>/<int:camera>/<int:programa>')
def video_feed_new_webcam_yolo(serial_number, camera, programa):
    """ Transmite vídeo (cru ou processado com YOLO) da câmera especificada """
    cam_key = None
    ...
    # Inicia a thread YOLO caso ainda não exista
    if cam_key not in processing_threads:
        t = threading.Thread(target=yolo_processing_thread, args=(cam_key,), daemon=True)
        t.start()
        processing_threads[cam_key] = t

    # === Geração dos frames para o navegador ===
    def generate_frames():
        while True:
            # Pega o último frame processado se disponível
            frame = processed_frames.get(cam_key)

            # Se YOLO ainda não processou nada, mostra o frame cru
            if frame is None:
                frame = get_latest_webcam_frame(cam_key)
                if frame is not None:
                    frame = cv2.resize(
            ...
            time.sleep(0.03)  # 30 FPS no navegador

    return Response(generate_frames(),
                    mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/video_feed_webcam_yolo_pessoas/<path:serial_number>/<int:camera>/<int:programa>')
def video_feed_new_webcam_yolo_pessoas(serial_number, camera, programa):
    """ Transmite vídeo (cru ou processado com YOLO) da câmera especificada """
    cam_key = None
    ...
    # Inicia a thread YOLO caso ainda não exista
    if cam_key not in processing_threads:
        t = threading.Thread(target=yolo_processing_thread_operadores, args=(cam_key,), daemon=True)
        t.start()
        processing_threads[cam_key] = t
    ...
    def generate_frames():
        while True:
            # Pega o último frame processado se disponível
            frame = processed_frames_pessoas.get(cam_key)
...
@app.route('/video_feed_webcam_yolo_carrinho/<path:serial_number>/<int:camera>/<int:programa>')
def video_feed_new_webcam_yolo_carrinho(serial_number, camera, programa):
    """ Transmite vídeo (cru ou processado com YOLO) da câmera especificada """
    cam_key = None
    ...
    # Inicia a thread YOLO caso ainda não exista
    if cam_key not in processing_threads:
        t = threading.Thread(target=yolo_processing_thread_carrinho, args=(cam_key,), daemon=True)
        t.start()
        processing_threads[cam_key] = t
    ...
    def generate_frames():
        while True:
            # Pega o último frame processado se disponível
            frame = processed_frames_carrinho.get(cam_key)
...
facial_pause = False
facial_pause_lock = threading.Lock()
########################## cam 2 Reconhecimento facial #############################################3
ultimo_nome_processado = ""
matricula_atual = "000001"
processed_frames_facial = {}
processing_threads_facial = {}
def facial_processing_thread(cam_key):
    """Thread contínua que aplica o facial e salva o frame processado"""
    print(f"[FACIAL] Iniciando monitoramento para {cam_key}")
    global ultimo_nome_processado, facial_pause, matricula_atual,frame_anotado_facial
    ...
    while True:
        try:
            ...
            frame_anotado = frame_anotado_facial#, ultimo_nome_processado, matricula_atual = facial.reconhecer_api(frame)
            processed_frames_facial[cam_key] = frame_anotado

        except Exception as e:
            print(f"[FACIAL] Erro processando {cam_key}: {e}")
            time.sleep(0.5)


def facial_processing_thread_cam2(cam_key):
    """Thread contínua que aplica o facial e salva o frame processado"""
    print(f"[FACIAL] Iniciando monitoramento para {cam_key}")
    global ultimo_nome_processado,facial_pause, matricula_atual, frame_anotado_facial
    while True:
        try:
            ...
            frame_anotado_facial, ultimo_nome_processado, matricula_atual = facial.reconhecer_api(frame)
            # Armazena o resultado anotado
            processed_frames_facial[cam_key] = frame_anotado_facial

        except Exception as e:
            print(f"[FACIAL] Erro processando {cam_key}: {e}")
            time.sleep(0.5)
######################################################################################################
def inicializa_facial_cam_2(cam_key):
     f = threading.Thread(target=facial_processing_thread_cam2, args=(cam_key,), daemon=True)
     f.start()

@app.route('/video_feed_webcam_facial/<path:serial_number>')
def video_feed_new_webcam_facial(serial_number):
    """ Transmite vídeo (cru ou processado com FACIAL) da câmera especificada """
    cam_key = None
    ...
    # Inicia a thread YOLO caso ainda não exista
    if cam_key not in processing_threads_facial:
        t = threading.Thread(target=facial_processing_thread, args=(cam_key,), daemon=True)
        t.start()
        processing_threads_facial[cam_key] = t

    # === Geração dos frames para o navegador ===
    def generate_frames():
        while True:
            # Pega o último frame processado se disponível
            frame = processed_frames_facial.get(cam_key)
            
            # Se ainda não processou nada, mostra o frame cru
            if frame is None:
                frame = get_latest_webcam_frame(cam_key)
                if frame is not None:
                    frame = cv2.resize(
...
################################### secao reconhecimento facial ##########################################

import threading

def converter_tempo_para_decimal(tempo_str):
    try:
...
def registrar_desempenho_thread(
    matricula,
    turno,
    tempo_no_berco,
    camera,
    ...
    try:
        tempo_no_berco = converter_tempo_para_decimal(tempo_cluster_no_berco)
        # ⚠️ conexão criada DENTRO da thread
        db = get_db()
        cursor = db.cursor()

        sql = """
        INSERT INTO desempenho_operacional
        (matricula, turno, tempo_no_berco_segundos, camera, programa)
        VALUES (%s, %s, %s, %s, %s)
        """
        ...
        print(f"[DB][ERRO] registrar_desempenho: {e}")
...
# Inicia a thread de registro de desempenho
threading.Thread(
    target=registrar_desempenho_thread,
    args=(matricula_atual,turno_atual,tempo_cluster_no_berco, 1, 1),
    daemon=True
).start()


@app.route('/reconhecer/<path:serial_number>', methods=['POST'])
def reconhecer_api(serial_number):
...
def gerar_embedding_seguro(imagem_bytes, queue):
    try:
        import cv2
        import numpy as np
        import face_recognition

        file_bytes = np.frombuffer(imagem_bytes, np.uint8)
        ...
        # 🔒 LIMITA RESOLUÇÃO (ESSENCIAL)
        h, w = imagem.shape[:2]
        if max(h, w) > 1600:
            scale = 1600 / max(h, w)
            imagem = cv2.resize(imagem, None, fx=scale, fy=scale)
        ...
        queue.put(encodings[0])

    except Exception:
        # ⚠️ Qualquer erro vira None
        queue.put(None)


PASTA_ROSTOS_NEW = "rostos"

from multiprocessing import Process, Queue
@app.route('/cadastrar_foto_new', methods=['POST'])
def cadastrar_foto_new():
    global facial_pause

    with facial_pause_lock:
        facial_pause = True

    if not os.path.exists(PASTA_ROSTOS_NEW):
        os.makedirs(PASTA_ROSTOS_NEW)
    ...
    if not nome or not matricula or not setor or not funcao:
    ...
    # 🔒 BLOQUEIO DE TAMANHO DE ARQUIVO
    if len(imagem_bytes) > 5 * 1024 * 1024:
        return jsonify({"erro": "Imagem muito grande (máx 5MB)"}), 400

    queue = Queue()
    processo = Process(
        target=gerar_embedding_seguro,
        args=(imagem_bytes, queue)
    )
    ...
    try:
        ...
        if processo.is_alive():
            processo.terminate()
            return jsonify({"erro": "Timeout no processamento facial"}), 400

        embedding = queue.get()

        if embedding is None:
            return jsonify({"erro": "Não foi possível gerar embedding facial"}), 400

        # ===== SALVANDO =====
        file_bytes = np.frombuffer(imagem_bytes, np.uint8)
        imagem = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)

        filename_base = secure_filename(matricula)
        caminho_npy = os.path.join(PASTA_ROSTOS_NEW, filename_base + ".npy")
        caminho_jpg = os.path.join(PASTA_ROSTOS_NEW, filename_base + ".jpg")

        np.save(caminho_npy, embedding)
        cv2.imwrite(caminho_jpg, imagem)
        ...
        cursor.execute("""
            ...
            (matricula, nome, setor, funcao, caminho_imagem, caminho_embedding)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (matricula, nome, setor, funcao, caminho_jpg, caminho_npy))
...
@app.route('/cadastrar_foto', methods=['POST'])
def cadastrar_foto():
    pasta = PASTA_ROSTOS
    if not os.path.exists(pasta):
        os.makedirs(pasta)
    ...
    try:
        # ======== 1️⃣ Caso venha imagem enviada via formulário ========
        if 'imagem' in request.files and request.files['imagem'].filename != '':
            imagem_arquivo = request.files['imagem']
            file_bytes = np.frombuffer(imagem_arquivo.read(), np.uint8)
            imagem = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)

        # ======== 2️⃣ Caso contrário, usa o último frame da câmera ========
        else:
            global last_frame
            if last_frame is None:
                return jsonify({"erro": "Nenhuma imagem enviada e nenhum frame disponível da câmera"}), 400
            imagem = last_frame.copy()

        # ======== 3️⃣ Processa a imagem ========
        imagem_rgb = cv2.cvtColor(imagem, cv2.COLOR_BGR2RGB)
        rostos = face_recognition.face_encodings(imagem_rgb)

        if rostos:
            nome_arquivo = secure_filename(nome)
            caminho_npy = os.path.join(pasta, nome_arquivo + ".npy")
            caminho_jpg = os.path.join(pasta, nome_arquivo + ".jpg")

            # Salva vetor facial e imagem original
            np.save(caminho_npy, rostos[0])
            cv2.imwrite(caminho_jpg, imagem)
...
@app.route('/cadastrar_foto_gpu', methods=['POST'])
def cadastrar_foto_gpu():
    pasta = PASTA_ROSTOS
    if not os.path.exists(pasta):
        os.makedirs(pasta)
    ...
    try:
        # ======== 1️⃣ Imagem enviada pelo formulário ========
        if 'imagem' in request.files and request.files['imagem'].filename != '':
            imagem_arquivo = request.files['imagem']
            file_bytes = np.frombuffer(imagem_arquivo.read(), np.uint8)
            imagem = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)

        # ======== 2️⃣ Senão, usa o último frame da câmera ========
        else:
            global last_frame
            if last_frame is None:
                return jsonify({"erro": "Nenhuma imagem enviada e nenhum frame disponível da câmera"}), 400
        ...
        # ======== 3️⃣ Detectar rostos com InsightFace ========
...
if __name__ == '__main__':
    if(cam_tipo == "webcam"):
       ...
       # captura imagens do bercos opS8S9 -camera 1
       inicializa_yolo_cam_1(cam_1,1,1) 

       # captura imagens do carrinho buffer - camera 2
       inicializa_yolo_cam_2(cam_2,2,1) 

       # reconhecimento facial operadores - camera 2
       inicializa_facial_cam_2(cam_2) 

       # captura imagens dos operadores op S1, S2 e s4  - camera 3
       inicializa_yolo_cam_3(cam_3,3,1)    
       
       #incializa monitoramento
       inicializa_monitoramento() 
    else:
