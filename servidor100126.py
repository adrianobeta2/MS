    ...
    try:
        ...
        for i, (x, y, w, h,textura_tolerancia,pixel_tolerancia, threshold_cor,ponto_tolerancia) in enumerate(rois, start=1):
            ...
            if rede_neural == True:
                ...
                #imagem_anotada = None
                #status_rna  = monitorar_movimentos_rasp.analise_cabo_flex(modelo_yol,classe_ok,classe_nok,image,coordenadas,0.7)
...
processing_threads = {}
def yolo_processing_thread(cam_key, camera, programa):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key}")
    global cluster_no_berco,tempo_sem_objeto,contador_total_clusters_berco,tempo_cluster_no_berco,tempo_berco_1_ocupado,tempo_berco_2_ocupado,total_clusters_carrinho
    while True:
        try:
            frame = get_latest_webcam_frame(cam_key)
            if frame is not None:
...
def yolo_processing_thread_operadores(cam_key, camera, programa):
    """Thread contínua que aplica o YOLO e salva o frame processado"""
    print(f"[YOLO] Iniciando monitoramento para {cam_key}")
...
def obter_turno():
    ...
    turno1_fim    = time(16, 50)

    turno2_inicio = time(16, 50)
    turno2_fim    = time(22, 0)

    turno3_inicio = time(22, 0)
    turno3_fim    = time(7, 0)  
...
def monitorar_eventos():
    global cluster_no_berco, tempo_sem_objeto, total_clusters_carrinho, total_operadores
    ...
    while True:
        try:
            # --- CLUSTER NO CARRINHO ---
            #if (cluster_no_berco and total_clusters_carrinho > 3 and total_operadores > 1 and tempo_cluster_no_berco > 15) and (tempo_berco_1_ocupado == 0 or tempo_berco_2_ocupado == 0):
            if (cluster_no_berco and total_clusters_carrinho > 3  and tempo_cluster_no_berco > 15) and (tempo_berco_1_ocupado == 0 or tempo_berco_2_ocupado == 0):
            ...
            if (cluster_no_berco and tempo_cluster_no_berco > 25 ): #10 segundos
                if pode_disparar("cluster_berco"):
                    print("[ALERTA] Há produtos aguardando serem removidos do berço!")
                    tocar_beep("produto_berco.wav")

            # --- REGISTRO DE DESEMPENHO NO DB---
                    registrar_desempenho_thread(matricula_atual,tempo_cluster_no_berco,"1","1")

            # --- SEM PRODUTO NO BERÇO ---
            if tempo_sem_objeto > 200 and not cluster_no_berco:# and total_operadores > 1:
               if pode_disparar("ops8_aguardando_producao"):
                  print("[ALERTA] Tempo excessivo sem produto testado!")
                  tocar_beep("ops8_aguardando_producao.wav")

            if tempo_berco_1_ocupado > 200: #
                if pode_disparar("berco_1_ocupado"):
                    print("[ALERTA] Berço 1 ocupado por muito tempo!")
                    tocar_beep("berco1.wav")
            if tempo_berco_2_ocupado > 200: #
...
def yolo_processing_thread_cam2(cam_key_cam2, camera_cam2, programa_cam2):
    ...
    global total_clusters_carrinho
    while True:
        try:
            ...
            # Aplica o processamento YOLO (sua função)
            frame_anotado, total_clusters_carrinho = monitorar_movimentos.monitor_movimento_carrinho(frame_cam2,camera_cam2,programa_cam2)

            # Armazena o resultado anotado
            processed_frames[cam_key_cam2] = frame_anotado
...
def video_feed_new_webcam_yolo(serial_number, camera, programa):
    ...
    if cam_key not in processing_threads:
        t = threading.Thread(target=yolo_processing_thread, args=(cam_key, camera, programa), daemon=True)
        t.start()
...
def video_feed_new_webcam_yolo_pessoas(serial_number, camera, programa):
    ...
    # Inicia a thread YOLO caso ainda não exista
    if cam_key not in processing_threads:
        t = threading.Thread(target=yolo_processing_thread_operadores, args=(cam_key, camera, programa), daemon=True)
        t.start()
        processing_threads[cam_key] = t
...
def facial_processing_thread(cam_key):
    ...
    global ultimo_nome_processado, facial_pause, matricula_atual
...
def registrar_desempenho_thread(
    ...
    try:
        ...
        (matricula, tempo_no_berco_segundos, camera, programa)
        VALUES (%s, %s, %s, %s)
        """
...
threading.Thread(
    ...
    args=(matricula_atual,tempo_cluster_no_berco, 1, 1),
    daemon=True
).start()
...
def cadastrar_foto_new():
    ...
    try:
        ...
        # ===== PROCESSAMENTO FACIAL SEGURO =====
        imagem_rgb = cv2.cvtColor(imagem, cv2.COLOR_BGR2RGB)
        rostos = face_recognition.face_encodings(imagem_rgb)

        if not rostos:
            return jsonify({"erro": "Erro ao gerar embedding facial"}), 400
